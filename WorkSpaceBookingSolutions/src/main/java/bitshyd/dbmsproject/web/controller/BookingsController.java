...
public class BookingsController extends HttpServlet {
	...
	try {
		...
		else if (request.getSession().getAttribute("UserRole").toString().equalsIgnoreCase("Staff")) {
			staffId =Integer.parseInt(request.getSession().getAttribute("UserID").toString());
		}
		WorkSpaceBookingDao objBookingsDao=new WorkSpaceBookingDao();
		ArrayList<WorkSpaceBooking> arrFutureBookings=objBookingsDao.getAllCustomerBookings(customerId, staffId, false, true, true, true);
		ArrayList<WorkSpaceBooking> arrRunningBookings=objBookingsDao.getAllCustomerBookings(customerId, staffId, true, false, true, true);
		ArrayList<WorkSpaceBooking> arrPastBookings=objBookingsDao.getAllCustomerBookings(customerId, staffId, true, true, false, false);
