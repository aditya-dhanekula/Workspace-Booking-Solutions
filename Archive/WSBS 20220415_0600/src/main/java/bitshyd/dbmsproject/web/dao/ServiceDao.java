...
public class ServiceDao {
	...
	public ArrayList<Service> getAllServices() {
		...
		try {
			...
			PreparedStatement st = con.prepareStatement(
					"Select s.ServiceID, s.ServiceName, s.ServicePrice, s.ServiceStatusID, ss.StatusName as ServiceStatus, s.TimeCreate, s.TimeUpdate "
	...
	public ArrayList<Service> getAllStaffServices(int UserId) {
		...
		try {
			...
			// System.out.println("Connection Established");
			PreparedStatement st = con.prepareStatement(
					"Select * from services where serviceID in (select serviceID from Staffservices where StaffID = ?)");
			st.setInt(1, UserId);
			// System.out.println(st.toString());
			// System.out.println("Before Query:" + st.toString());
			ResultSet rs = st.executeQuery();
			...
			while (rs.next()) {
				...
				objService.setServiceStatusID(rs.getInt("ServiceStatusID"));
				objService.setTimeCreate(rs.getDate("TimeCreate"));
				objService.setTimeUpdate(rs.getDate("TimeUpdate"));
				arrServices.add(objService);
			}
			rs.close();
			return arrServices;
	...
	public ArrayList<Service> getAllStaffServicesOfIds(String serviceIds) {
		...
		try {
			...
			// System.out.println("Connection Established");
			PreparedStatement st = con.prepareStatement(
					"Select * from services where serviceID in (" + serviceIds + ");");
			// System.out.println(st.toString());
			// System.out.println("Before Query:" + st.toString());
			ResultSet rs = st.executeQuery();
			// System.out.println("After Query");
	...
	public ArrayList<Service> getAllActiveServicesWithStaffSelectedStatus(int staffId) {
		...
		try {
			...
			// System.out.println("Connection Established");
			PreparedStatement st = con.prepareStatement(
					"Select SRV.ServiceID, SRV.ServiceName, SRV.ServicePrice, "
	...
	public int addService(String ServiceName, int ServicePrice ) {
		Connection con = null;
		try {
			con = Database.getConnection();
			// System.out.println("Connection Established");
			PreparedStatement st = con.prepareStatement("Insert into services (ServiceName, ServicePrice, ServiceStatusID, TimeCreate, TimeUpdate ) "
					+ "Values (?,?,1, now(), now() ); ");
			st.setString(1, ServiceName);
			st.setInt(2, ServicePrice);
			// System.out.println("Before Query:" + st.toString());
	...
	public int updateService(int ServiceId, String ServiceName, int ServicePrice ) {
		Connection con = null;
		try {
			con = Database.getConnection();
			// System.out.println("Connection Established");
			PreparedStatement st = con.prepareStatement("Update services Set ServiceName = ?, ServicePrice = ?, TimeUpdate = now() where ServiceID=?;");
			st.setString(1, ServiceName);
			st.setInt(2, ServicePrice);
